...
- **مدیریت کاربران در Settings هنوز UI ندارد** — `CreateUserHandler` ساخته و تست شده، ولی صفحه‌ای برای دیدن/بایگانی‌کردن/تغییر رمز کاربران از داخل برنامه هنوز نیست؛ فقط از راه دیگری (مثلاً دیتابیس مستقیم یا ابزار جدا) قابل استفاده است.
- **نمایشگر Audit هنوز UI ندارد** — ردیف‌های audit (شامل `sales.credit.over-limit-approved` که حالا به یک Admin واقعی وصل است) نوشته می‌شوند ولی صفحه‌ای برای دیدنشان از داخل برنامه نیست.
