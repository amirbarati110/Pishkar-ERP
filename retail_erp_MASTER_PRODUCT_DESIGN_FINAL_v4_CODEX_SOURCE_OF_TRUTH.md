...
# پیوست ز — هویت، ورود و مجوز (Identity/Login/Permission)

> §۱۵.۲۴ فاز ۰ می‌گفت پایه‌ی برنامه شامل Identity/Permission است؛ §۱۰ (طرح مرحله‌ی اول) صریح‌تر بود: «مدیر اولیه در راه‌اندازی ساخته می‌شود و رمز عبور Hash می‌شود». این قدم تا نشست ۱۴۰۵/۰۶/۲۶ ساخته نشده بود — به‌جایش هر Use Case از یک `PilotUserContext` با GUID ثابت استفاده می‌کرد و تأیید مدیر برای عبور از سقف اعتبار (§۶.۷) هیچ اعتبارسنجی واقعی نداشت (پیوست د.۲). این پیوست تصمیم‌های اجرایی این قدم را ثبت می‌کند.

## ز.۱ قوانین قفل‌شده

1. **رمز عبور با PBKDF2-HMAC-SHA256 هش می‌شود** (`ERP.Domain.Identity.PasswordHash`) — ۲۱۰٬۰۰۰ تکرار (توصیه‌ی OWASP ۲۰۲۳)، salt تصادفی به ازای هر رمز، مقایسه با زمان‌ثابت (`CryptographicOperations.FixedTimeEquals`) تا حمله‌ی زمان‌سنجی ممکن نباشد. رمز خام هرگز ذخیره یا لاگ نمی‌شود.
2. **هیچ‌کس رمز مشترک ندارد** (§۱۵.۳ «PIN شخصی، رمز مشترک ممنوع»). دو نقش: `Admin`، `Cashier`. اولین کاربر همیشه Admin است و فقط یک بار — با `RegisterFirstAdminHandler`، که اگر حتی یک کاربر از قبل باشد رد می‌شود، حتی اگر UI راه‌اندازی اشتباهی دوباره باز شود.
3. **پیام‌های ورود عمداً کلی‌اند.** نام کاربری اشتباه، رمز اشتباه، و حساب بایگانی‌شده هر سه همان یک پیام «نام کاربری یا رمز عبور اشتباه است» را می‌دهند — نمی‌شود از پیام فهمید کدام نام کاربری واقعی است.
4. **تأیید مدیر برای عبور از سقف اعتبار (§۶.۷) حالا واقعی است.** دکمه‌ی «تأیید مدیر و ثبت نسیه» دیگر مستقیم فاکتور را کامل نمی‌کند؛ یک پنجره باز می‌شود، مدیر نام‌کاربری/رمز خودش را تایپ می‌کند، `VerifyAdminCredentialHandler` نقش را هم چک می‌کند (نه فقط رمز درست، باید `Role == Admin` هم باشد) — همان محدودیتی که پیوست د.۲/و.۲ به‌عنوان کار نشده ثبت کرده بودند، اینجا بسته شد.
5. **بررسی نقش در لایه‌ی Application انجام می‌شود، نه فقط پنهان‌کردن دکمه در UI** (§۱۰) — مثلاً `CreateUserHandler` نقش کاربر فراخوان را از دیتابیس دوباره می‌خواند و چک می‌کند، به همان چیزی که کلاینت ادعا می‌کند اعتماد نمی‌کند.
6. **حساب بایگانی‌شده نمی‌تواند وارد شود**، حتی با رمز درست (`User.VerifyPassword` اول `Status == Active` را چک می‌کند).

## ز.۲ محدودیت‌های آگاهانه (باید بعداً تکمیل شوند)

- **نمایشگر Audit هنوز UI ندارد** — ردیف‌های audit (شامل `sales.credit.over-limit-approved` که حالا به یک Admin واقعی وصل است، و از ۱۴۰۵/۰۷/۰۲ هر تغییر روی خود کاربران) نوشته می‌شوند ولی صفحه‌ای برای دیدنشان از داخل برنامه نیست.
- **ویزارد راه‌اندازی کسب‌وکار ساخته نشده** — نام فروشگاه/شعبه/انبار هنوز مقادیر پیش‌فرض با GUID ثابت هستند (`RetailSetupDefaults`)؛ این نشست فقط حساب مدیر را اضافه کرد، نه بقیه‌ی تنظیمات اولیه.
- **بازیابی رمز فراموش‌شده وجود ندارد** — اگر تنها مدیر رمزش را فراموش کند، راه بازیابی از داخل برنامه نیست.

## ز.۳ اجرا و تست

- Domain: `UserId`, `UserRole`, `UserStatus`, `PasswordHash`, `User` (`src/ERP.Domain/Identity/`) — ۱۶ تست.
- Application: `RegisterFirstAdmin`, `SignIn`, `VerifyAdminCredential`, `CreateUser` (`src/ERP.Application/Identity/`) — ۱۲ تست با repository حافظه‌ای.
- دیتابیس: Migration V009 (جدول `APP_USER`، `UQ_APP_USER_USERNAME`)؛ `FirebirdUserRepository`، `FirebirdIdentityService` — ۵ تست روی Firebird واقعی، شامل یک تست رقابت دو ثبت هم‌زمان که محدودیت دیتابیس را (نه فقط چک لایه‌ی Application) تأیید می‌کند.

## ز.۴ نقش‌ها و مجوزهای هر کاربر (تصمیم کاربر ۱۴۰۵/۰۷/۰۲)

> کاربر صریحاً پرسید کدام کارها باید مجوز جدا داشته باشند؛ پاسخ: «مرجوعی و تعویض»، «اصلاح فاکتور ثبت‌شده»، «دیدن بهای تمام‌شده و سود» و «ویرایش و حذف مشتری» — هر چهار مورد با هم، به‌عنوان چهار سوییچ جدا برای هر صندوق‌دار، پیش‌فرض روشن. بقیه‌ی کارهای حساس (تعریف کالا/قیمت اصلی، موجودی اول دوره، وارد کردن از اکسل، انبارها، پشتیبان‌گیری/بازیابی، خود «کاربران») مخصوص مدیر ماند — این‌ها هیچ‌وقت سوییچ نداشتند و نیازی هم به آن نبود.

1. **جدول حق‌ها یک‌جا و صریح است** (`ERP.Domain.Identity.AccessRight` + `AccessRules.Allows`) — نه پراکنده در هر Handler. `Admin` همه‌ی حق‌ها را دارد؛ `Cashier` همیشه `SellAndTill` را دارد و چهار حق دیگر را فقط با سوییچ مربوطه (`CashierPermissions`، یک `[Flags] enum`، پیش‌فرض `All`).
2. **بررسی همیشه در لایه‌ی Application و روی داده‌ی تازه‌خوانده‌شده است** (§۱۰، همان قانون ز.۱.۵) — `AccessPolicy.CheckAsync` کاربر را از دیتابیس دوباره می‌خواند، نه از چیزی که در حافظه یا در ادعای کلاینت است. یعنی اگر مدیر یک اجازه را همین الان بردارد، همان کار بعدی صندوق‌دار رد می‌شود — نه از سراغ ورود بعدی.
3. **مهاجرت بدون تغییر رفتار** (V026): ستون `APP_USER.PERMISSIONS` با مقدار پیش‌فرض ۱۵ (`CashierPermissions.All`) اضافه شد — هیچ صندوق‌داری با ارتقا هیچ حقی را که از قبل داشت از دست نداد.
4. **پنهان‌کردن هم انجام شد، ولی هرگز به‌جای بررسی سرور** — کارت منوی مدیرمحور اصلاً در «میز کار» لیست نمی‌شود (`NavigationMap.VisibleCards`)، و «قیمت اصلی کالا هم به‌روز شود» فقط برای کسی که `ManageCatalog` دارد نشان داده می‌شود. اگر کسی مسیر دیگری برای رسیدن به همان دستور پیدا کند، سرور همان‌جا با پیام فارسی رد می‌کند.
5. **بهای تمام‌شده و سود، نه پنهان‌کردن یک ستون در کلاینت، بلکه حذف مقدار در پاسخ سرور است** — `GetLineEditInfoHandler`/`CostAwareStockCardHandler` مقدار `null` برمی‌گردانند وقتی `ViewCostAndProfit` نیست؛ عددی که هیچ‌وقت به کلاینت نرسیده را نمی‌شود از کنسول مرورگر یا یک کلاینت دست‌ساز خواند.
6. **هر تغییر روی خود کاربران (ساخت، ویرایش نقش/سوییچ، رمز تازه، غیرفعال‌کردن) یک ردیف Audit جدا می‌گیرد** (§۱۵.۴ «Security Action») — رمز هرگز در آن نوشته نمی‌شود، فقط این‌که عوض شده.
7. **آخرین مدیر فعال هرگز حذف یا تنزل نمی‌شود** (`LastActiveManager`) — وگرنه فروشگاه بدون هیچ مدیری می‌ماند و راه بازگشتی نیست.

### اجرا و تست

- Domain: `AccessRight`, `CashierPermissions`, `AccessRules` (`src/ERP.Domain/Identity/AccessRight.cs`), `User.Can`/`SetPermissions`/`ChangeRole`.
- Application: `AccessPolicy`, `IAccessChecker`, `ManageUsers.cs` (List/Update/ResetPassword/Archive) — تست‌های واحد شامل هر چهار سوییچ، آخرین‌مدیر و بی‌درنگ‌بودن رد شدن.
- Persistence: Migration V026، `FirebirdAccessChecker`، `FirebirdUserRepository.ListAsync` — `AccessEndToEndTests` روی Firebird واقعی: صندوق‌دار رد می‌شود برای کار مدیر، هر سوییچ که مدیر خاموش می‌کند از همان فراخوان بعدی اثر می‌کند، رمز و نقش و اجازه در گزارش‌ها می‌مانند.
- Desktop: صفحه‌ی «کاربران» (منوی «تنظیمات و سیستم») — لیست، فرم ساخت/ویرایش با نقش و چهار سوییچ، رمز تازه، حذف=غیرفعال‌کردن با پرسش اول؛ کارت‌های مدیرمحور در «میز کار» پنهان از کسی که حقش را ندارد؛ چک‌باکس قیمت اصلی در صفحه‌ی فروش هم همین‌طور.
- Desktop: `AuthWindow` (`راه‌اندازی اولیه`/`ورود`، جایگزین `PilotUserContext`)، دروازه‌ی «تأیید مدیر» روی دکمه‌ی عبور از سقف اعتبار در صفحه‌ی فروش — ۹ تست view-model (۷ برای Setup/Login، ۲ برای تأیید مدیر با اعتبارنامه‌ی درست/غلط/غیرمدیر).

# پیوست ح — Import Center کالا (§۱۴ قدم ۷، §۱۵.۴)

> ۱۴۰۵/۰۶/۲۹. §۱۵.۴ می‌خواهد: «فایل Excel کالا، بارکد، قیمت و موجودی را پیش‌نمایش و با گزارش خطای فارسی وارد کرد و در صورت نیاز کل Import Batch را Rollback کرد». نیمی از این قدم (خواندن OOXML بدون کتابخانه‌ی بیرونی، اعتبارسنجی ردیف‌به‌ردیف) از قبل در کد بود و در همین نشست کشف شد — این پیوست نیمه‌ی باقی‌مانده (نوشتن اتمیک در دیتابیس + صفحه) و کل تصمیم را با هم ثبت می‌کند تا نشست بعدی دوباره از صفر کشفش نکند.

## ح.۱ قوانین قفل‌شده

1. **خواندن اکسل بدون هیچ کتابخانه‌ی بیرونی.** `ERP.Infrastructure.Spreadsheets.OpenXmlSpreadsheetReader` مستقیم روی فایل zip/XML فرمت OOXML کار می‌کند (`System.IO.Compression` + `System.Xml.Linq`، هر دو در .NET پایه). تصمیم آگاهانه بود: ClosedXML (MIT) در برابر EPPlus (از نسخه ۵ به بعد Polyform Noncommercial — برای محصول تجاری رایگان نیست) در برابر NPOI (Apache 2.0، سنگین‌تر) — ولی چون یک پیاده‌سازی خانگی از قبل کار می‌کرد و تست داشت، نیازی به افزودن هیچ‌کدام نبود.
2. **اعتبارسنجی (Prepare) هیچ I/O ندارد.** `ProductImportPreparer.Prepare` یک متد static خالص است: `SpreadsheetTable` + `CatalogLookupSnapshot` می‌گیرد، `ProductImportPreview` برمی‌گرداند. نه دیتابیس می‌خواند نه می‌نویسد — یعنی پیش‌نمایش همیشه رایگان و بی‌خطر است، حتی قبل از هر تصمیمی برای واردکردن.
3. **نوشتن، یک تراکنش برای کل دسته.** `CommitProductImportHandler` یک `FirebirdUnitOfWork` باز می‌کند، همه‌ی ردیف‌ها را در حافظه‌ی همان تراکنش می‌نویسد، و فقط در پایان یک‌بار `CommitAsync` صدا می‌زند. اگر وسط راه ردیفی رد شود (مثلاً بارکدی که بین Preview و Commit توسط دستگاه دیگری ثبت شده)، تابع بدون فراخوانی Commit برمی‌گردد و `FirebirdUnitOfWork.DisposeAsync` خودش رول‌بک می‌کند — نه فقط ردیف خراب، کل دسته، حتی ردیف‌های قبل از آن. این رفتار روی Firebird واقعی تست شده (`CommitProductImportServiceTests`)، نه فقط با یک repository حافظه‌ای که اصلاً تراکنش ندارد.
4. **موجودی اولیه اختیاری و per-row است.** اگر ستون «موجودی اولیه» پر باشد، «بهای تمام‌شده اولیه (تومان)» هم باید معتبر باشد — وگرنه خطای روشن به‌جای صفر بی‌صدا. کالای بدون این دو ستون فقط بدون موجودی ساخته می‌شود (دقیقاً مثل ساختن دستی از صفحه‌ی «کالا»).
5. **پیام خطا همیشه شماره‌ی ردیف واقعی فایل را دارد** (نه اندیس صفرمبنای داخلی)، چون کاربر باید بتواند همان ردیف را در اکسل خودش پیدا کند.

## ح.۲ محدودیت‌های آگاهانه (باید بعداً تکمیل شوند)

- فقط `.xlsx` (OOXML) — فرمت باینری قدیمی `.xls` پشتیبانی نمی‌شود.
- نگاشت‌پذیری هدر ستون‌ها نیست — نام ستون باید دقیقاً «نام کالا»، «دسته‌بندی» و... باشد؛ مترادف یا زبان دیگر پذیرفته نمی‌شود.
- هر کالا حداکثر یک بارکد از فایل می‌گیرد (ستون «بارکد» تکی)؛ افزودن چند بارکد به یک کالا از این مسیر ممکن نیست.
- وارد کردن فقط برای کالای تازه است، نه به‌روزرسانی کالای موجود (قیمت/موجودی یک کالای از قبل ساخته‌شده از این فایل تغییر نمی‌کند؛ فقط ردیف‌های تازه اضافه می‌شوند).

## ح.۳ اجرا و تست

- Infrastructure: `OpenXmlSpreadsheetReader` (از قبل بود).
- Application: `ProductImportPreparer` (ستون‌های موجودی اولیه امروز اضافه شد)، `CommitProductImportHandler` (تازه) — ۹ تست Preparer + ۳ تست Commit با repository حافظه‌ای.
- Persistence: `FirebirdRetailSetupService.ExecuteAsync(CommitProductImportCommand)` — ۲ تست روی Firebird واقعی، یکی صریحاً رول‌بک اتمیک کل دسته را ثابت می‌کند.
- Desktop: صفحه‌ی «وارد کردن از اکسل» (منوی اصلی، `ImportCenterPage`) با `FileOpenPicker`، پیش‌نمایش ردیف‌به‌ردیف (قرمز = خطا)، دکمه‌ی «وارد کردن» فقط وقتی هیچ خطایی نمانده فعال است — ۳ تست view-model.

# پیوست ط — شیفت صندوق پایه (§۱۴ قدم ۸، §۱۵.۵)

> ۱۴۰۵/۰۶/۲۹. `CashShift` در §۶ (مدل داده پایه) به‌عنوان موجودیت اصلی نام برده شده بود ولی هیچ‌کدام از چهار لایه ساخته نشده بودند — حتی کاشی «وضعیت صندوق» در میز کار به‌صراحت غیرفعال بود. این پیوست تصمیم‌های اجرایی این قدم را ثبت می‌کند.

## ط.۱ قوانین قفل‌شده

1. **یک شیفت باز به‌ازای هر انبار.** `OpenCashShiftHandler` قبل از باز کردن، `ICashShiftRepository.GetOpenAsync` را چک می‌کند؛ اگر شیفتی باز باشد، رد می‌شود با پیام روشن («اول همان را ببندید»)، نه خطای فنی.
2. **`Money` هرگز منفی نیست، پس «کسری/اضافه» از نوع دیگری است.** تلاش اول برای `Variance` از `Money` استفاده می‌کرد و بلافاصله با یک تست شکست: `Money.FromRials` هر عدد منفی را رد می‌کند (این محدودیت از قبل، برای قیمت/جمع، درست است) و کسری صندوق ذاتاً می‌تواند منفی باشد. `Variance` حالا یک `long?` ریال با علامت است، نه `Money?` — نمونه‌ی روشنی از این‌که یک نوع دامنه‌ی درست برای یک مفهوم، لزوماً برای مفهوم دیگر درست نیست.
3. **جمع فروش نقدی هرگز از ورودی کاربر یا از خودِ `CashShift` نمی‌آید.** `CloseCashShiftHandler` در لحظه‌ی بستن، از `ISaleReadReader.ListCompletedAsync(shift.OpenedAtUtc, closedAtUtc)` می‌خواند و فقط فاکتورهای `PaymentMethod.Cash` را جمع می‌زند — همان الگوی موجود «امروز» در میز کار، نه یک محاسبه‌ی تازه‌ی موازی.
4. **مرز ماژول‌ها (§۵) رعایت شد**: `ERP.Domain.Cashiering` هیچ ارجاعی به `ERP.Domain.Sales` ندارد؛ ترکیب داده‌ی دو ماژول فقط در لایه‌ی Application، داخل `CloseCashShiftHandler`/`GetCashShiftStatusHandler`، اتفاق می‌افتد.
5. **وضعیت زنده، نه فقط لحظه‌ی باز شدن.** `GetCashShiftStatusHandler` هر بار که صدا زده شود، فروش نقدی را از باز شدن شیفت تا همین الان دوباره می‌خواند — کاشی «وضعیت صندوق» و صفحه‌ی شیفت هر دو یک عدد واقعی و به‌روز نشان می‌دهند، نه یک عدد ذخیره‌شده در لحظه‌ی باز شدن.

## ط.۲ محدودیت‌های آگاهانه (باید بعداً تکمیل شوند)

- **فروش هنوز گِیت پشت شیفت باز نیست.** صندوق‌دار می‌تواند بدون باز کردن شیفت هم بفروشد. قفل‌کردن این مسیر (هشدار؟ جلوگیری کامل؟) یک تصمیم UX جدا است که عمداً بیرون نگه داشته شد — تغییر رفتار صفحه‌ی فروش (که کاملاً تست‌شده است) بدون طرح تأییدشده ریسک غیرضروری بود.
- بازیابی رمز/تأیید مدیر برای بستن شیفت (مثلاً وقتی کسری بزرگ است) هنوز نیست — هرکسی که وارد شده می‌تواند شیفت را ببندد.
- تطبیق روش‌های غیرنقدی (کارت‌خوان، چک) در شیفت نیست — فقط نقدی شمرده می‌شود.
- گزارش تاریخچه‌ی شیفت‌های گذشته (لیست/چاپ) نیست؛ فقط شیفت باز فعلی قابل دیدن است.
- **یک نقص واقعی که در بررسی کیفی ۱۴۰۵/۰۶/۲۹ پیدا و همان روز رفع شد**: جمع «فروش نقدی» شیفت (`CloseCashShiftHandler`/`GetCashShiftStatusHandler`) از `ISaleReadReader.ListCompletedAsync` می‌خواند که اصلاً پارامتر انبار ندارد — یعنی سراسری بود، نه محدود به انبار همان شیفت. چون `CashShift` خودش دقیقاً بر اساس «یک شیفت باز به‌ازای هر انبار» طراحی شده، این یک ناهماهنگی واقعی بود. رفع: یک متد تازه به‌جای تغییر متد قدیمی — `ISaleReadReader.ListCompletedByWarehouseAsync(warehouseId, from, to)` — اضافه شد؛ نسخه‌ی سراسری برای میز کار/«امروز» دقیقاً همان‌طور که بود باقی ماند (آن‌جا عمداً سراسری است). با یک تست تازه روی Firebird واقعی (دو انبار، دو شیفت هم‌زمان باز، فروش نقدی جدا) ثابت شد.

## ط.۳ اجرا و تست

- Domain: `CashShift`, `CashShiftId`, `CashShiftStatus` (`src/ERP.Domain/Cashiering/`) — ۷ تست، شامل تست‌های کسری/اضافه/دقیقاً برابر.
- Application: `OpenCashShift`, `CloseCashShift`, `GetCashShiftStatus` (`src/ERP.Application/Cashiering/`) — ۹ تست با repository حافظه‌ای و `ISaleReadReader` جعلی.
- Persistence: Migration V010 (جدول `CASH_SHIFT`)، `FirebirdCashShiftRepository`، `FirebirdCashieringService` — ۲ تست روی Firebird واقعی، یکی با یک فروش نقدی و یک فروش کارتی واقعی که تطبیق درست را ثابت می‌کند.
- Desktop: صفحه‌ی «شیفت صندوق» (منوی اصلی + کاشی «وضعیت صندوق» فعال‌شده در میز کار) — ۵ تست view-model.

# پیوست ی — سند حسابداری حداقلی (§۱۴ قدم ۹، §۶.۲۳، §۱۰.۵)

> ۱۴۰۵/۰۶/۲۹. بازخورد مستقیم کاربر («سند حسابداری داغونه»، پیوست الف-۲ ردیف ۷) از همان ابتدای نشست ثبت شده بود ولی تا این‌جا هیچ سندی تولید نمی‌شد. §۱۰.۵ (Auto Accounting) می‌گفت «Business Operation سند تولید می‌کند؛ کاربر عملیاتی Debit/Credit نمی‌بیند» — این پیوست همان قدم را می‌بندد، عمداً در کوچک‌ترین شکل درستش.

## ی.۱ قوانین قفل‌شده

1. **Chart of Accounts ثابت و هاردکد است**، نه قابل‌تنظیم. شش حساب: صندوق، کارتخوان، حساب‌های دریافتنی (نسیه)، چک‌های دریافتنی، درآمد فروش، مالیات بر ارزش‌افزوده پرداختنی. §۱۰.۶ «Accounting Mapping — Rules قابل تنظیم» فصل ۱۰ کامل است، صریحاً خارج از محدوده‌ی مرحله‌ی اول.
2. **هر سند باید موازنه داشته باشد، وگرنه اصلاً وجود ندارد.** `JournalEntry.Create` جمع بدهکار و بستانکار هر سطر را قبل از ساخت چک می‌کند؛ نامساوی یعنی `DomainException`، نه یک سند نادرست ذخیره‌شده.
3. **اصلاحیه سند تازه‌ی خودش را می‌گیرد؛ سند فاکتور اصلی دست‌نخورده می‌ماند** — دقیقاً همان قانون پیوست و.۱ برای خودِ `Sale`، این‌بار برای `JournalEntry`.
4. **فروش کاملاً تخفیف‌خورده (مبلغ صفر) هیچ سندی نمی‌گیرد.** اولین تلاش این را نادیده گرفته بود و مستقیم به خطای «سطر با مبلغ صفر» در `Money`/`JournalLine` برخورد کرد — نه پولی جابه‌جا شده نه درآمدی به دست آمده، پس چیزی برای ثبت نیست.
5. **در POS دیده نمی‌شود** (§۶.۲۳). دکمه‌ی «سند» فقط روی ردیف‌های «فاکتورهای امروز» است، یک پنجره‌ی فقط-نمایشی باز می‌کند، و هیچ فیلد قابل‌ویرایشی ندارد — «کاربر عملیاتی Debit/Credit نمی‌بیند» یعنی حتی وقتی می‌بیند، دست به آن نمی‌زند.

6. **(۱۴۰۵/۰۷/۰۲ — ممیزی روند کار)** هر عملیاتی که پول یا کالا جابه‌جا می‌کند سند می‌گیرد، نه فقط فروش و مرجوعی:
   «دریافت از مشتری» (بدهکار صندوق/کارتخوان، بستانکار دریافتنی)، «موجودی اول دوره» و Import کالا (بدهکار موجودی کالا،
   بستانکار «سرمایه و مانده‌های افتتاحیه» — حساب دهم)، و «مانده اول دوره»ی مشتری (بدهکار دریافتنی، بستانکار همان حساب).
7. **اصلاحیه، بخش فروشِ سندِ فاکتوری را که جایش را می‌گیرد با یک سند «برگشت» جدا خنثی می‌کند** (`SaleCorrectionReversal`):
   دقیقاً آینه‌ی سطرهای واقعاً ثبت‌شده، بدون جفت بهای تمام‌شده (کالا جابه‌جا نشده). سند اصلی همچنان دست‌نخورده است (قانون ۱۵).
   بدون این، درآمد هر دو فاکتور در دفتر می‌ماند.

## ی.۲ محدودیت‌های آگاهانه (باید بعداً تکمیل شوند)

- **بهای تمام‌شده کالای فروش‌رفته (COGS) نیست.** سند فقط سمت درآمد را می‌زند (بدهکار روش پرداخت، بستانکار درآمد/مالیات)؛ سطر «بدهکار COGS / بستانکار موجودی کالا» که هزینه‌ی واقعی FIFO مصرف‌شده را ثبت کند ساخته نشده — نیاز به گرفتن آن هزینه از `StockLedger.ConsumeFifo` و پاس‌دادن به لایه‌ی حسابداری، در همان تراکنش تکمیل فروش.
- تخفیف و هزینه/خدمات حساب جدا نمی‌گیرند؛ هر دو در همان یک سطر «درآمد فروش» خالص می‌شوند.
- دسترسی به دکمه‌ی «سند» بر اساس نقش محدود نیست (همان محدودیت §۱۵.۲ ثبت‌شده در چک‌لیست ردیف «د»-۵) — هرکسی که وارد شده می‌تواند سند را ببیند، نه فقط مدیر/حسابدار.
- دفتر معین، تراز آزمایشی، سود‌وزیان، ترازنامه (General Ledger, Trial Balance, P&L, Balance Sheet — §۱۰.۴) نیستند؛ کاملاً خارج از محدوده‌ی مرحله‌ی اول.

## ی.۳ اجرا و تست

- Domain: `JournalEntry`, `JournalLine`, `JournalEntryId`, `AccountCode`, `JournalSourceType` (`src/ERP.Domain/Accounting/`) — ۶ تست، شامل رد سند نامتوازن و سطر با مبلغ صفر.
- Application: `SaleJournalEntryFactory` (نگاشت روش پرداخت → حساب بدهکار)، `GetSaleJournalEntry`، و اتصال مستقیم داخل `CompleteSaleHandler`/`CompleteSaleCorrectionHandler` در همان تراکنش تکمیل فروش — ۵ تست factory + ۲ تست یکپارچه (فروش واقعی → سند موازنه‌شده؛ اصلاحیه → سند جدا، بدون دست‌زدن به سند اصلی).
- Persistence: Migration V011 (جداول `JOURNAL_ENTRY`/`JOURNAL_LINE`، insert-only)، `FirebirdJournalEntryRepository`، `FirebirdAccountingService` — ۲ تست روی Firebird واقعی با یک فروش کارتی مالیات‌دار واقعی.
- Desktop: دکمه‌ی «سند» روی هر ردیف «فاکتورهای امروز» → پنجره‌ی فقط-نمایشی سند حسابداری — ۱ تست view-model.

# پیوست ک — پیش‌نمایش و چاپ رسید پایه (§۱۴ قدم ۱۰، §۱۴.۱۴، §۱۵ معیار ۸)

> ۱۴۰۵/۰۶/۲۹. «چاپ فاکتور» در پیوست الف-۲ (بازخورد ۸) ❌ بود؛ دکمه‌اش روی صفحه غیرفعال با نوشته‌ی «به‌زودی». این پیوست تصمیم‌های اجرایی همین قدم را ثبت می‌کند.

## ک.۱ قوانین قفل‌شده

1. **چاپ هیچ داده‌ی تازه‌ای نمی‌سازد.** پیش‌نمایش/چاپ فقط `IGetSaleDetailsHandler` (همان کوئری که خودِ صفحه‌ی فروش برای دیدن یک فاکتور استفاده می‌کند) را دوباره صدا می‌زند — نه فروش، نه پرداخت، نه موجودی، نه سند حسابداری را لمس می‌کند. یعنی چاپ مجدد همیشه امن است؛ همان الگوی «سند» (پیوست ی) و «اصلاح» (پیوست و).
2. **چاپ واقعی است، نه یک شبیه‌سازی.** بعد از بررسی، `System.Drawing.Printing.PrintDocument` (GDI+ کلاسیک، بسته‌ی `System.Drawing.Common`) انتخاب شد به‌جای سطح جدیدتر چاپ WinUI/WindowsAppSDK — دلیل: بدون نیاز به پنجره‌ی انتخاب چاپگر یا API چاپ تازه‌ای که این پروژه تا الان استفاده نکرده بود، مستقیم روی چاپگر پیش‌فرض ویندوز چاپ می‌کند. طبق §۱۶ «چاپگر واقعی برای توسعه اجباری نیست» — چاپگر پیش‌فرض می‌تواند «Microsoft Print to PDF» باشد؛ نیازی به سخت‌افزار چاپگر رسید برای آزمایش این مسیر نیست.
3. **قالب رسید ثابت و ساده است**: نام فروشگاه (فعلاً جای‌گیر «پیشکار ERP»، چون Business Setup هنوز ساخته نشده)، شماره فاکتور، تاریخ/ساعت، مشتری، ردیف‌های کالا، جمع کالاها، تخفیف/هزینه/مالیات (هرکدام فقط اگر غیرصفر باشد)، قابل‌پرداخت، روش پرداخت، توضیحات.
4. **دکمه‌ی چاپ هم در خلاصه‌ی «فروش با موفقیت ثبت شد» هست هم روی هر ردیف «فاکتورهای امروز»** — یکی مسیر تازه‌ثبت‌شده، دیگری چاپ مجدد؛ هر دو از همان یک Command عبور می‌کنند (`OpenReceiptPreviewCommand`) تا رفتارشان جدا نشود.

## ک.۲ محدودیت‌های آگاهانه (باید بعداً تکمیل شوند)

- **Print Profile واقعی نیست** (§۱۴.۱۱): نه انتخاب ۵۸mm/۸۰mm، نه لوگو، نه QR، نه تعداد کپی؛ یک قالب متنی ثابت با فونت مونواسپیس.
- **بدون پنجره‌ی انتخاب چاپگر** — همیشه چاپگر پیش‌فرض سیستم؛ برای انتخاب چاپگر دیگر، کاربر باید چاپگر پیش‌فرض ویندوز را عوض کند.
- **Per-workstation Profile نیست** (§۱۴.۱۳) — نیاز به موجودیت Workstation دارد که هنوز در مدل داده نیست.
- **`ERP.LocalAgent` به‌عنوان یک Host/پردازه‌ی جدا ساخته نشد** — منطق چاپ مستقیم داخل `ERP.Desktop` است. ساخت یک Local Agent واقعی (پردازه‌ی پس‌زمینه‌ی مستقل پشت یک Contract، برای چاپ/Backup/Device) یک تصمیم معماری بزرگ‌تر است که عمداً به بعد موکول شد.
- **بدون Retry خودکار روی خطای چاپ** — اگر چاپ ناموفق باشد، پیام فارسی نشان داده می‌شود و کاربر باید دوباره دکمه را بزند؛ Retry خودکار نیست (که طبق §۱۶ همان رفتار قابل‌قبولی است چون تکرار عملیات مالی نیست، فقط یک چاپ).

## ک.۳ اجرا و تست

- Presentation: `ReceiptLineRow`، افزوده‌های `SalesWorkspaceViewModel.Receipt.cs` (`OpenReceiptPreviewAsync` از `SaleDetails`، بدون کوئری یا لایه‌ی تازه‌ای در Application/Persistence — همه‌چیز از `IGetSaleDetailsHandler` موجود ساخته شد) — ۲ تست view-model (پیش‌نمایش بعد از تکمیل، چاپ مجدد بدون فروش تازه).
- Desktop: دکمه‌ی «چاپ رسید» در خلاصه‌ی تکمیل، دکمه‌ی «چاپ» روی «فاکتورهای امروز»، پنجره‌ی پیش‌نمایش، و چاپ واقعی با `System.Drawing.Printing` در `SalesPage.xaml.cs`.

# پیوست ل — Backup محلی، بازیابی آزمایشی و سلامت سیستم (§۱۴ قدم ۱۱، §۱۵ معیار ۱۱)

> ۱۴۰۵/۰۶/۲۹. آخرین قدم از ۱۲ قدم ترتیب ساخت مرحله اول که پیش از تست‌های End-to-End (قدم ۱۲) مانده بود. `BackupRecord` در §۶ به‌عنوان موجودیت اصلی نام برده شده بود.

## ل.۱ قوانین قفل‌شده

1. **Backup با همان بسته‌ی FirebirdClient گرفته می‌شود، نه یک ابزار بیرونی.** `FbBackup`/`FbRestore` از `FirebirdSql.Data.Services` (همان NuGet که همه‌جای این پروژه برای اتصال دیتابیس استفاده می‌شود) کار gbak را انجام می‌دهند — بدون شل‌کردن به `gbak.exe`، بدون پکیج تازه.
2. **«سالم» یعنی واقعاً بازیابی و خوانده شده، نه فقط این‌که فایل روی دیسک هست.** `IVerifyBackupHandler` نسخه پشتیبان را در یک فایل دیتابیس یک‌بارمصرف (پوشه‌ی موقت سیستم) بازیابی می‌کند، `SCHEMA_MIGRATIONS` را می‌شمارد، سپس فایل موقت را حذف می‌کند. تا وقتی این اتفاق نیفتاده، `BackupRecord.Status` روی `NotVerified` می‌ماند — نه یک حدس خوش‌بینانه.
3. **رکورد فقط بعد از این‌که فایل واقعاً ساخته شد نوشته می‌شود.** اگر `FbBackup` شکست بخورد، هیچ ردیفی در `BACKUP_RECORD` نوشته نمی‌شود — یک نسخه پشتیبان ناموفق هرگز به یک ردیفی که بعداً کسی به آن اعتماد کند تبدیل نمی‌شود.
4. **Health Check از جدول واقعی خودِ دیتابیس می‌خواند** (`SCHEMA_MIGRATIONS`)، نه از لیست Migration های داخل کد در حال اجرا — اگر این build با یک دیتابیس قدیمی‌تر/جدیدتر باز شده باشد، جواب همچنان درست است.
5. **یک باگ واقعی زمان ساخت پیدا شد و در کد مستند است:** بعد از `FbBackup.ExecuteAsync`/`FbRestore.ExecuteAsync`، فایل دیتابیس/نسخه پشتیبان بلافاصله آزاد نمی‌شود — Services API یک attachment جدا از pool معمولی `FbConnection` باز می‌کند و `FbConnection.ClearAllPools()` روی آن اثر ندارد. راه‌حل: یک حلقه‌ی صبر محدود (حداکثر ۱ ثانیه، هر ۱۰۰ میلی‌ثانیه یک تلاش برای باز کردن انحصاری فایل) قبل از هر عملیات بعدی روی همان فایل. بدون این، هر تست واقعی (و هر استفاده‌ی واقعی پشت‌سرهم از این صفحه) با خطای «فایل توسط پردازه‌ی دیگری استفاده می‌شود» شکست می‌خورد.

## ل.۲ محدودیت‌های آگاهانه (باید بعداً تکمیل شوند)

- فقط دیسک محلی — NAS/Cloud (§۱۵.۱۰) نیست.
- بدون رمزگذاری فایل `.fbk` (§۱۵.۱۱).
- بدون زمان‌بندی خودکار روزانه/هفتگی — فقط دستی، کاربر باید دکمه را بزند.
- بدون `Company.erpbackup` برای انتقال کامل به سیستم دیگر (§۱۵.۱۲) — کاملاً خارج از محدوده‌ی مرحله‌ی اول.
- `ERP.LocalAgent` به‌عنوان یک Host/پردازه‌ی جدا ساخته نشد؛ منطق Backup مستقیم داخل `ERP.Persistence`/`ERP.Desktop` است — همان تصمیم پیوست ک برای چاپ.
- **یک نقص واقعی که در بررسی کیفی ۱۴۰۵/۰۶/۲۹ پیدا و تا حد امکان همان روز رفع شد**: `ReleaseFileAsync` در `FirebirdBackupEngine` حداکثر ۱ ثانیه (۱۰ تلاش، هر ۱۰۰ میلی‌ثانیه) صبر می‌کند تا فایل آزاد شود؛ قبلاً بعد از این مدت کاملاً بی‌سروصدا (بدون خطا، بدون لاگ) برمی‌گشت. رفع: حالا `bool` برمی‌گرداند و اگر فایل موقت بازیابی‌شده در پایان «بررسی اعتبار» هنوز آزاد نشده باشد، یک هشدار فارسی به متن نتیجه اضافه می‌شود، نه این‌که ناپدید شود. خودِ عدد ۱ ثانیه عمداً دست‌نخورده ماند — تبدیلش به شکست صریح یا عدد دیگر، بدون امکان اجرای واقعی روی یک سیستم واقعی‌تر (و احتمالاً کندتر) برای سنجش عدد درست، ریسک ایجاد خطای الکی روی سیستم‌های کندتر داشت.

## ل.۳ اجرا و تست

- Domain: `BackupRecord`, `BackupRecordId`, `BackupVerificationStatus` (`src/ERP.Domain/Backups/`) — ۵ تست.
- Application: `CreateBackup`, `VerifyBackup`, `GetSystemHealth`, `IBackupEngine` (مرز به Firebird واقعی) (`src/ERP.Application/Backups/`) — ۸ تست با engine جعلی.
- Persistence: Migration V012 (جدول `BACKUP_RECORD`)، `FirebirdBackupEngine` (`FbBackup`/`FbRestore` واقعی)، `FirebirdBackupRecordRepository`، `FirebirdMigrationStatusReader`، `FirebirdBackupService` — ۲ تست **روی Firebird واقعی** که یک نسخه پشتیبان واقعی می‌گیرند و واقعاً بازیابی‌اش می‌کنند (نه Fake) — تنها جایی در این نشست که کل مسیر یک قابلیت، از دکمه تا فایل روی دیسک، به‌صورت end-to-end با ابزار واقعی (نه فقط repository حافظه‌ای) تأیید شده است.
- Desktop: صفحه‌ی «پشتیبان‌گیری» (منوی اصلی) با سلامت سیستم، «گرفتن نسخه پشتیبان»، «بررسی اعتبار» — ۴ تست view-model.

# پیوست م — تست End-to-End، Rollback، Idempotency و ارتقای Migration (§۱۴ قدم ۱۲، §۱۳)

> ۱۴۰۵/۰۶/۲۹. قدم دوازدهم و آخرِ ترتیب ساخت مرحله اول. برخلاف قدم‌های ۱ تا ۱۱ (هرکدام یک قابلیت تازه)، این قدم درباره‌ی *آزمودن* چیزی است که از قبل ساخته شده — یک agent تحقیقاتی جدا، بدون نوشتن کد، هر ۵ بند §۱۳ را که هنوز پوشش نداشتند بررسی و با آدرس فایل/تست دقیق گزارش کرد؛ نتیجه صفر از پنج بود. این پیوست همان گزارش و کاری که رویش انجام شد را ثبت می‌کند.

## م.۱ قوانین قفل‌شده

1. **«سناریوی خودکار حیاتی» یک تست واحد است، نه مجموعه‌ای از تست‌های جدا.** `CriticalPathTests.cs` دقیقاً همان ۶ قدم §۱۳ را به همان ترتیب پشت سر هم اجرا می‌کند (کالا → موجودی اولیه → شیفت → فروش نقدی → کاهش موجودی → چاپ مجدد) و بین هر قدم واقعیت دیتابیس را چک می‌کند — هر بخش قبلاً جدا هم تست شده بود، ولی این تنها جایی است که ثابت می‌کند با هم کار می‌کنند.
2. **تست Rollback باید یک نوشتن «موفق» وسط تراکنش را هم پس بگیرد، نه فقط ثابت کند نوشتن آخر رد شده.** تست تک‌خطی قبلی (`CompleteSaleTests.cs`) فقط یک خط داشت. تست تازه دو خط دارد: خط اول موجودی کافی دارد و واقعاً `ConsumeFifo`+`SaveAsync` رویش اجرا می‌شود، خط دوم رد می‌شود — و موجودی خط اول هم دقیقاً به حالت قبل برمی‌گردد، چون همه‌چیز یک تراکنش Firebird است که تا آخر Commit نشده.
3. **Idempotency یعنی «رد شدن تمیز»، نه «شماره‌ی فاکتور سوخته نمی‌شود».** `CompleteSaleHandler` شماره‌ی فاکتور را قبل از چک «آیا هنوز پیش‌نویس است» می‌کشد؛ یعنی تکمیل دوباره‌ی یک فاکتورِ از قبل تکمیل‌شده یک شماره را بی‌مصرف می‌گذارد. این یک تصمیم آگاهانه است، نه باگ — شکاف در شماره‌گذاری فاکتور یک رفتار شناخته‌شده و قابل‌قبول است؛ چیزی که واقعاً اهمیت دارد (موجودی دوبار کم نشود، سند حسابداری دوبار پست نشود) تست و تأیید شد.
4. **Migration Test از نسخه‌ی قبلی باید از لیست واقعی migration های برنامه استفاده کند، نه یک لیست دستیِ جدا** — تا اگر کسی migration تازه‌ای اضافه کرد، این تست خودش را با آن به‌روز نگه دارد. به همین دلیل `FirebirdDatabaseBootstrapper.Migrations` از یک لیست inline داخل متد به یک فیلد static عمومی جابه‌جا شد.

## م.۲ محدودیت‌های آگاهانه (باید بعداً تکمیل شوند)

- **تست Performance ساخته نشد.** سند هدف مشخصی دارد (کوئری روزمره < ۲۰۰ms، ثبت فروش محلی < ۵۰۰ms) ولی نیاز به طراحی جدا برای داده‌ی نمونه‌ی حجیم دارد؛ عمداً بیرون از این افزایش ماند.
- **«بررسی دستی» RTL، متن فارسی، وضوح ۱۳۶۶×۷۶۸، Keyboard-only و Touch target ها انجام نشد** — این بند صریحاً می‌گوید «بررسی دستی»؛ Developer Mode ویندوز در این محیط خاموش است و برنامه‌ی WinUI واقعی در کل این نشست هرگز روی صفحه اجرا و دیده نشده — همان محدودیت شناخته‌شده از ابتدای نشست.

## م.۳ اجرا و تست

- Persistence: `tests/ERP.Persistence.Tests/EndToEnd/CriticalPathTests.cs` (۱ تست، روی Firebird واقعی)، `tests/ERP.Persistence.Tests/Sales/CompleteSaleRollbackTests.cs` (۱ تست)، `tests/ERP.Persistence.Tests/Sales/CompleteSaleIdempotencyTests.cs` (۱ تست)، `tests/ERP.Persistence.Tests/Migrations/MigrationRunnerTests.cs` (۱ تست تازه اضافه‌شده).
- مجموع تست‌های پروژه در پایان این افزایش: ۴۰۲ (Domain ۱۲۹، Application ۱۰۸، Infrastructure ۱، Persistence ۷۰، Desktop ۹۲، Architecture ۲).

## م.۴ بررسی کیفی نهایی و دو رفع واقعی (۱۴۰۵/۰۶/۲۹، ادامه)

بعد از قدم ۱۲، به درخواست کاربر («بهترین خودت رو انجام بده») یک بررسی کیفی روی کل تغییرات این نشست انجام شد: بیلد کامل از صفر (پاک‌کردن هر `bin`/`obj`) از طریق `RetailERP.slnx`، بعد بررسی کد. تلاش اول برای بررسی با ۸ دستیار موازی به‌خاطر محدودیت نرخ (rate limit) شکست خورد؛ به‌جایش خودِ من مستقیم بررسی کردم و دو نقص واقعی پیدا شد که همان روز رفع شدند:

1. **جمع فروش نقدی شیفت سراسری بود، نه محدود به انبار** (پیوست ز) — یک متد تازه (`ListCompletedByWarehouseAsync`) اضافه شد، بدون تغییر رفتار سراسری «امروز»ی میز کار. تست تازه: دو انبار، دو شیفت هم‌زمان باز.
2. **آزادسازی فایل بعد از Backup/Restore کاملاً بی‌صدا بود** (پیوست ل) — حالا نتیجه‌ی «بررسی اعتبار» اگر فایل موقت هنوز آزاد نشده باشد یک هشدار فارسی دارد.

مجموع تست‌های پروژه بعد از این دو رفع: ۴۰۳ (یک تست تازه برای مورد ۱؛ مورد ۲ یک تغییر رفتاری کوچک داخل تست‌های Backup موجود بود، تست تازه‌ای اضافه نکرد).

---

# پیوست ن — مرجوعی و تعویض (§۶.۲۵، §۶.۲۶)

> ۱۴۰۵/۰۶/۲۹. جزئیات و وضعیت هر بند در بخش «ل» فایل `docs/checklists/sales-and-base-design-vs-source-of-truth.md`. اینجا فقط **تصمیم‌های قفل‌شده** که نشست بعدی نباید برگرداند:

1. **کاربر انتخاب کرد:** مرجوعی + تعویض؛ مقصد کالا هر سه (برگشت به انبار، خراب، ضایعات)؛ روش بازپرداخت انتخاب آزاد در لحظه‌ی مرجوعی.
2. **کالای برگشتی به انبار با بهای اصلی همان فروش برمی‌گردد، نه بهای روز.** برای این، هزینه‌ی FIFO هر خط فروش در لحظه‌ی تکمیل ذخیره می‌شود (`SALE_LINE_COST`). بهای فروش‌های قدیمی حدس زده نمی‌شود؛ فقط «خراب/ضایعات» می‌پذیرند.
3. **خراب و ضایعات هزینه‌ی کالا را در «بهای تمام‌شده» نگه می‌دارند** (زیان واقعی است)؛ فقط «برگشت به انبار» بدهکار موجودی / بستانکار بهای تمام‌شده می‌زند.
4. **آخرین تکه‌ی یک سطر باقیمانده‌ی رند را می‌گیرد** تا مرجوعی‌های تکه‌تکه دقیقاً کل فاکتور شوند.
5. **مرجوعی سند مستقلِ تغییرناپذیر است** (قانون ۱۵): اشتباه با سند تازه جبران می‌شود. فاکتوری که مرجوعی دارد اصلاح نمی‌شود و فاکتور اصلاح‌شده فقط از طریق اصلاحیه‌اش مرجوع می‌شود.
6. **نقدیِ برگشتی جدا از فروش نقدی در شیفت ذخیره می‌شود** (`Money` منفی نمی‌شود؛ مرجوعی نقدی می‌تواند از فروش نقدی همان روز بیشتر باشد).
7. **تعویض = دو سند کامل مستقل** (مرجوعی + فروش جایگزین) با راهنمای مابه‌التفاوت در پنجره‌ی پرداخت، نه یک سند ترکیبی — چون پرداخت ترکیبی (§۶.۱۵) هنوز نیست. اگر پرداخت ترکیبی ساخته شد، می‌شود مرجوعی را مستقیم از فاکتور جدید کم کرد.
8. **«خدمات/هزینه»ی فاکتور (مثلاً ارسال) در مرجوعی به انتخاب کاربر پس داده می‌شود** (تصمیم کاربر ۱۴۰۵/۰۷/۰۲: «قابل انتخاب باشه بخواد پس بده یا نده»): پیش‌فرض پس داده **نمی‌شود**؛ اگر تیک زده شود با مالیاتش برمی‌گردد؛ حداکثر یک بار برای هر فاکتور؛ مرجوعیِ فقط خدمات (بدون کالا، مثلاً ارسالی که انجام نشد) مجاز است.

---

# پیوست س — ساختار منو (بخش ← کارت ← صفحه)، لیست کالاها و کاردکس کالا (۱۴۰۵/۰۶/۲۹ – ۳۱)

> منبع: ۲۶ عکس از برنامه‌ی «باران» که خود کاربر داد + سه پرسش و پاسخ + تحقیق. **جزئیات و وضعیت هر بند در `docs/checklists/sales-and-base-design-vs-source-of-truth.md` بخش‌های «م» تا «م-۵»**؛ این پیوست فقط تصمیم‌های قفل‌شده را نگه می‌دارد تا برنگردند.

1. **ساختار باران، ظاهر خودمان** (§۳.۱۳ از اول «Domain-based» می‌خواست): منوی راست فقط بخش‌های اصلی (میز کار، فروش و صندوق، کالا و قیمت‌گذاری، انبار و موجودی، تنظیمات و سیستم)؛ هر بخش یک صفحه‌ی کارتی؛ هر کارت یک صفحه‌ی جدا. تنها منبع ساختار: `NavigationMap`.
2. **کارتِ ساخته‌نشده اصلاً نشان داده نمی‌شود** (نه «به‌زودی»، نه کم‌رنگ).
3. **شکل صفحه‌ی لیست:** لیست + جست‌وجوی نیم‌کلمه + گروه‌ها کنار + جمع‌ها + نوار دکمه‌ی جدید F2 · ویرایش F3 · حذف F4 · کاردکس F7 · خروج Esc. فیلتر/ترتیب/سقف در دیتابیس (سقف ۳۰۰ ردیف با پیام صریح).
4. **«حذف (F4)» همان است که کاربر خواست** (پیشنهاد «بایگانی» را نپذیرفت)؛ زیرش کالا **بایگانی** می‌شود، با تأیید اجباری، تا سابقه‌ی فروش بماند (قانون ۱۵ / §۳.۱۰). این تضاد با §۳.۱۴ («حذف: Shortcut تک‌کلیدی آسان ممنوع») **آگاهانه** با تأیید جبران شده است.
5. **Stepper «مرحله ۱ از ۳» برداشته شد** (در سند نبود؛ ویزارد برای کار طولانی/یک‌باره است، ثبت کالا کار پرتکرار §۳.۱۴).
6. **ویرایش کالا:** نام، کد، دسته، قیمت. بارکد و واحد اصلی از فرم ویرایش عوض نمی‌شوند (موجودی با واحد شمرده شده؛ بارکد فقط اضافه‌شدنی).
7. **کاردکس کالا = «تعدادی-ریالی»:** وارده · صادره · مانده (تعداد) و فی · مبلغ · مبلغ مانده (به **بهای تمام‌شده‌ی FIFO**، نه قیمت فروش). بها فقط از منبع ثبت‌شده می‌آید (لایه‌ی اول دوره، `SALE_LINE_COST` از V014، بهای اصلی مرجوعی). **حرکتِ بی‌بها «—» است و ارزش مانده از آن‌جا به بعد نامعلوم؛ هیچ‌وقت حدس زده نمی‌شود.**
8. **از کاردکس وارد فاکتور شدن:** ردیف فاکتور ← «دیدن فاکتور» با اصلاح/سند/چاپ/مرجوعی (همان فرمان‌های قبلی)؛ ردیف مرجوعی ← فاکتور اصلی؛ «بازگشت» به همان کاردکس (پوسته زنده می‌ماند).
9. **قاعده‌ی RTL:** `HorizontalAlignment="Left"` در چیدمان راست‌به‌چپ به **راست** می‌رود؛ برای «چپِ فیزیکی» `Right` بگذارید (سه باگ قدیمی صفحه‌ی فروش با همین رفع شد).

**هنوز ساخته نشده:** لیست انبارها و اشخاص، چاپ لیست/کاردکس/اتیکت، ستون‌های تخفیف و حد سفارش، نمایش/بازگردانی حذف‌شده‌ها، بازسازی بهای فروش‌های قبل از V014 (فقط با برچسب «بازسازی‌شده»)، میانگین متحرک در کاردکس.
